...
class Chat : AppCompatActivity() {
    ...
    private fun getIntentData() {
        if (intent != null) {
            if (intent.hasExtra("name") && intent.hasExtra("id")) {
                Toast.makeText(this, "Details ${intent.getStringExtra("name")}   ${intent.getStringExtra("id")}", Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun getAllData() {
        firebaseFirestore.collection("IChat")
            ...
            .addSnapshotListener { querySnapshot, firebaseExeption ->


                if (firebaseExeption != null) {
                    Log.d("EIREBASE_EXC", "Error:" + firebaseExeption.message)
                } else {
